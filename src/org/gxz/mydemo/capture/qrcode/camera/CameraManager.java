...
package org.gxz.mydemo.capture.qrcode.camera;
...
public final class CameraManager {
	...
	public Rect getFramingRect() {
		...
		if (framingRect == null) {
			...
			// ===========diff===========
			int realWidth;
			if (captureSettings.isPortrait()) {
				realWidth = display.getWidth() / 3 * 2;
			} else {
				realWidth = display.getHeight() / 3 * 2;
			}

			int realHeight = realWidth;
			int realLeft = (display.getWidth() - realWidth) / 2;
			int realTop = (display.getHeight() - realHeight) / 2;

			framingRect = new Rect(realLeft, realTop, realLeft + realWidth,
					realTop + realHeight);
