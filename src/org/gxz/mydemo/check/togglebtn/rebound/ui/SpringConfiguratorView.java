...
package org.gxz.mydemo.check.togglebtn.rebound.ui;
...
import org.gxz.mydemo.check.togglebtn.rebound.OrigamiValueConverter;
import org.gxz.mydemo.check.togglebtn.rebound.Spring;
import org.gxz.mydemo.check.togglebtn.rebound.SpringConfig;
import org.gxz.mydemo.check.togglebtn.rebound.SpringConfigRegistry;
import org.gxz.mydemo.check.togglebtn.rebound.SpringListener;
import org.gxz.mydemo.check.togglebtn.rebound.SpringSystem;
...
/**
 * The SpringConfiguratorView provides a reusable view for live-editing all
...
public class SpringConfiguratorView extends FrameLayout {
	...
	public SpringConfiguratorView(Context context, AttributeSet attrs,
		...
		mRevealerSpring.setCurrentValue(1).setEndValue(1)
		...
		mSpringSelectorSpinner
				.setOnItemSelectedListener(new SpringSelectedListener());
	...
	/**
	 * Programmatically build up the view hierarchy to avoid the need for
	 * resources.
	 * 
	...
	private View generateHierarchy(Context context) {
		...
		params = Util.createLayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
				Util.dpToPx(300, resources));
		...
		params = Util.createLayoutParams(Util.dpToPx(50, resources),
		...
		params = Util.createLayoutParams(Util.dpToPx(60, resources),
				Util.dpToPx(40, resources));
	...
	public void refreshSpringConfigurations() {
		Map<SpringConfig, String> springConfigMap = springConfigRegistry
				.getAllSpringConfig();
	...
	private class SpringSelectedListener implements
	...
	private class SeekbarListener implements SeekBar.OnSeekBarChangeListener {
		...
		public void onProgressChanged(SeekBar seekBar, int val, boolean b) {
			...
			if (seekBar == mTensionSeekBar) {
				float scaledTension = ((val) * tensionRange) / MAX_SEEKBAR_VAL
						+ MIN_TENSION;
				mSelectedSpringConfig.tension = OrigamiValueConverter
						.tensionFromOrigamiValue(scaledTension);
				String roundedTensionLabel = DECIMAL_FORMAT
						.format(scaledTension);
				mTensionLabel.setText("T:" + roundedTensionLabel);
			}

			if (seekBar == mFrictionSeekBar) {
				float scaledFriction = ((val) * frictionRange)
						/ MAX_SEEKBAR_VAL + MIN_FRICTION;
				mSelectedSpringConfig.friction = OrigamiValueConverter
						.frictionFromOrigamiValue(scaledFriction);
				String roundedFrictionLabel = DECIMAL_FORMAT
						.format(scaledFriction);
	...
	private void updateSeekBarsForSpringConfig(SpringConfig springConfig) {
		float tension = (float) OrigamiValueConverter
				.origamiValueFromTension(springConfig.tension);
		float tensionRange = MAX_TENSION - MIN_TENSION;
		int scaledTension = Math
				.round(((tension - MIN_TENSION) * MAX_SEEKBAR_VAL)
						/ tensionRange);

		float friction = (float) OrigamiValueConverter
				.origamiValueFromFriction(springConfig.friction);
		float frictionRange = MAX_FRICTION - MIN_FRICTION;
		int scaledFriction = Math
				.round(((friction - MIN_FRICTION) * MAX_SEEKBAR_VAL)
						/ frictionRange);
