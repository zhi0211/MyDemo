...
public class ExpandDragListView extends ExpandableListView {
	 ...
	 * menu出现，消失的速度
	 */
	private static final int MENU_VELCITY = 80;
	/**
	 * 屏幕宽度值。
	 */
	 ...
	 * 自身
	 */
	private ExpandDragListView mDragListView = this;
	/**
	 * 全局手势，主要用于click事件
	 */
	...
	public interface OnSingleClickListener {
		public void onContentClick(int groupId, int childId, HostRecord hr);
	}
	 ...
	 *            设置dragList监听事件
	 */
	public void setSingleClickLister(OnSingleClickListener clickListener) {
		this.clickListener = clickListener;
	}

	public ExpandDragListView(Context ctx, AttributeSet attrs) {
		super(ctx, attrs);
		this.ctx = ctx;
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			long pos = getExpandableListPosition(dragPosition
					- getFirstVisiblePosition());
			int type = getPackedPositionType(pos);
			groupId = getPackedPositionGroup(pos);
			childId = getPackedPositionChild(pos);
			// Log.d("================================", "type:" + type
			// + ";groupId:" + groupId + ";childId" + childId);
			if (type != 1)
				break;
			itemLockIndex = -1;
			item = (LinearLayout) getChildAt(dragPosition);
			if (item == null) {
				break;
			}
			if (viewIndex != null) {
				...
				if (dragPosition != -1)
					initValues(dragPosition, groupId, childId);
			}
			if (realMenuWidth != 0) {
	 ...
	 * 初始化被选中item
	 */
	private void initValues(int pos, int groupId, int childId) {
		if (item.getChildCount() != 2) {
			return;
		}
		// Log.d("====================","init values");
		hr.name = getExpandableListAdapter().getChild(groupId, childId)
				.toString();
		// Log.d("====================","hr.name:"+hr.name);
		realMenuWidth = menuWidth;
		// if (hr.hostType == DBConstants.HOST_TYPE_LOCALHOST)
		// realMenuWidth = 0;
		...
		// 左边缘的值赋值为menu宽度的负数
		contentLeftEdge = -realMenuWidth;
		contentRightEdge = 0;
		// content的leftMargin设置为右边缘的值，这样初始化时menu就变为不可见
		contentParams.leftMargin = 0;
		// 将content的宽度设置为屏幕宽度
		menu.getLayoutParams().width = realMenuWidth;
	...
	public boolean onTouchEvent(MotionEvent ev) {
		if (dragImageView != null && realMenuWidth != 0) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				} else if (!isMenuVisible && getScrollVelocity() != 0
						&& distanceX < -10) {
					lockScroll();
		...
		// 传递手势event
		listener.setView(groupId, childId);
	...
	private boolean shouldScrollToMenu() {
		return downX - UpX > realMenuWidth / 2
	...
	private boolean shouldScrollToContent() {
		return UpX - downX + screenWidth - realMenuWidth > screenWidth / 2
				|| getScrollVelocity() > SNAP_VELOCITY;
	}
	...
	private class itemOnGestureListener implements OnGestureListener {
		 ...
		 *            位置
		 */
		public void setView(int groupId, int childId) {
		...
		public boolean onSingleTapUp(MotionEvent e) {
			switch (e.getAction()) {
			case MotionEvent.ACTION_UP:
				if (itemLockIndex == -1 && viewIndex == null && childId != -1) {
					if (clickListener != null)
						clickListener.onContentClick(groupId, childId, hr);
				}
				break;
			default:
				break;
