package org.gxz.mydemo.datetime.date;
...
public class DatePickerDialog extends DialogFragment implements View.OnClickListener, DatePickerController {
    ...
    private void adjustDayInMonthIfNeeded(int month, int year) {
        ...
        int daysInMonth = DateTimeUtils.getDaysInMonth(month, year);
        if (day > daysInMonth) {
	...
	private void setCurrentView(int currentView, boolean forceRefresh) {
		...
		case MONTH_AND_DAY_VIEW:
			ObjectAnimator monthDayAnim = DateTimeUtils.getPulseAnimator(mMonthAndDayView, 0.9F, 1.05F);
			if (mDelayAnimation) {
            ...
            DateTimeUtils.tryAccessibilityAnnounce(mAnimator, mSelectDay);
            break;
		case YEAR_VIEW:
			ObjectAnimator yearAnim = DateTimeUtils.getPulseAnimator(mYearView, 0.85F, 1.1F);
			if (mDelayAnimation) {
			...
			DateTimeUtils.tryAccessibilityAnnounce(mAnimator, mSelectYear);
            break;
		}
	}
	...
	private void updateDisplay(boolean announce) {
        ...
        if (announce) {
            ...
            DateTimeUtils.tryAccessibilityAnnounce(mAnimator, fullDateText);
        }
	}
