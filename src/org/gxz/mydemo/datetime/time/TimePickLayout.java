package org.gxz.mydemo.datetime.time;
...
import org.gxz.mydemo.datetime.DateTimeUtils;
 ...
 */
public class TimePickLayout extends RelativeLayout implements
	...
	// private static final String KEY_CURRENT_ITEM_SHOWING =
	// "current_item_showing";
	...
	private boolean mCloseOnSingleTapHour;
	private boolean mCloseOnSingleTapSecond;
	private LinearLayout mSecondTextLayout;
	private boolean mIsSecondTextShow = true;
	...
	public interface OnTimeSetListener {
		 ...
		 */
		void onTimeSet(RadialPickerLayout view,boolean isSecondShow, int hourOfDay, int minute,
				int second);
	}

	public TimePickLayout(Context context, AttributeSet attrs) {
		super(context, attrs);
		LayoutInflater inflater = LayoutInflater.from(context);
		inflater.inflate(R.layout.time_picker_dialog, this, true);
		newInstance();
	}
	...
	public void setIs24Hour(boolean is24Hour) {
		mIs24HourMode = is24Hour;
	}

	// public static TimePickLayout newInstance(OnTimeSetListener callback,
	...
	public void newInstance() {
		...
		initialize(mCallback, calendar.get(Calendar.HOUR_OF_DAY),
				calendar.get(Calendar.MINUTE), calendar.get(Calendar.SECOND),
				true, false);
	}

	public void initialize(OnTimeSetListener callback, int hourOfDay,
			int minute, int second, boolean is24HourMode, boolean vibrate) {
		mCallback = callback;
	...
	public void setCloseOnSingleTapSecond(boolean closeOnSingleTapSecond) {
		mCloseOnSingleTapSecond = closeOnSingleTapSecond;
	}

	public void setCloseOnSingleTapHour(boolean closeOnSingleTapHour) {
		mCloseOnSingleTapHour = closeOnSingleTapHour;
	}

	public void setIsSecondShow(boolean isShow) {
		mIsSecondTextShow=isShow;
		if(mIsSecondTextShow){
			setSecondVisibility(View.VISIBLE);
		}else{
			setSecondVisibility(View.GONE);
			setCurrentItemShowing(HOUR_INDEX, true, false, true);
		}
	}
	
	private void setSecondVisibility(int visibility){
		mSecondTextLayout.setVisibility(visibility);
	}
	...
	public void init() {
		...
		mSecondPickerDescription = res
				.getString(R.string.second_picker_description);
		mSelectSeconds = res.getString(R.string.select_seconds);
		mBlue = res.getColor(R.color.time_blue);
		...
		mSecondTextLayout = (LinearLayout) findViewById(R.id.second_text_layout);
		...
		mTimePicker.initialize(getContext(), mInitialHourOfDay, mInitialMinute,
				mInitialSecond, mIs24HourMode, mVibrate);
		...
		mDoneButton = (TextView) findViewById(R.id.done_button);
		...
		if (mIs24HourMode) {
			...
			// RelativeLayout.LayoutParams paramsSeparator = new
	...
	private void onDoneButtonClick() {
		...
		if (mCallback != null) {
			mCallback.onTimeSet(mTimePicker,mIsSecondTextShow, mTimePicker.getHours(),
					mTimePicker.getMinutes(), mTimePicker.getSeconds());
		}
	}
	...
	private void updateAmPmDisplay(int amOrPm) {
		if (amOrPm == AM) {
			...
			DateTimeUtils.tryAccessibilityAnnounce(mTimePicker, mAmText);
			mAmPmHitspace.setContentDescription(mAmText);
		} else if (amOrPm == PM) {
			mAmPmTextView.setText(mPmText);
			DateTimeUtils.tryAccessibilityAnnounce(mTimePicker, mPmText);
	...
	public void onValueSelected(int pickerIndex, int newValue,
		...
		if (pickerIndex == HOUR_INDEX) {
			...
			if (mAllowAutoAdvance && autoAdvance) {
				if (mCloseOnSingleTapHour) {
					setCurrentItemShowing(MINUTE_INDEX, true, true, false);
				}
				announcement += ". " + mSelectMinutes;
			}
			DateTimeUtils.tryAccessibilityAnnounce(mTimePicker, announcement);
		} else if (pickerIndex == MINUTE_INDEX) {
			setMinute(newValue);
			String announcement = String.format("%d", newValue);
			if (mAllowAutoAdvance && autoAdvance) {
				if (mCloseOnSingleTapMinute && mIsSecondTextShow) {
	...
	private void setHour(int value, boolean announce) {
		...
		if (announce) {
			DateTimeUtils.tryAccessibilityAnnounce(mTimePicker, text);
		}
	}
	...
	private void setCurrentItemShowing(int index, boolean animateCircle,
		...
		if (index == HOUR_INDEX) {
			...
			mTimePicker.setContentDescription(mHourPickerDescription + ": "
					+ hours);
			if (announce) {
				DateTimeUtils.tryAccessibilityAnnounce(mTimePicker,
						mSelectHours);
			}
			...
			mTimePicker.setContentDescription(mSecondPickerDescription + ": "
					+ seconds);
			if (announce) {
				DateTimeUtils.tryAccessibilityAnnounce(mTimePicker,
						mSelectSeconds);
		...
		ObjectAnimator pulseAnimator = DateTimeUtils.getPulseAnimator(
	...
	private boolean isTypedTimeFullyLegal() {
		if (mIs24HourMode) {
			...
			return (values[0] >= 0 && values[1] >= 0 && values[1] < 60 && values[2] < 60);
		} else {
	...
	private void updateDisplay(boolean allowEmptyDisplay) {
		if (!allowEmptyDisplay && mTypedTimes.isEmpty()) {
			...
			String secondStr = (values[2] == -1) ? mDoublePlaceholderText
					: String.format(secondFormat, values[2]).replace(' ',
							mPlaceholderText);
			...
			mSecondView.setTextColor(mBlack);
			if (!mIs24HourMode) {
	...
	private int[] getEnteredTime(Boolean[] enteredZeros) {
		...
		int[] ret = { hour, minute, second, amOrPm };
		return ret;
	}
