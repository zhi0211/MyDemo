...
public class RadialPickerLayout extends FrameLayout implements OnTouchListener {
	...
	public void initialize(Context context, int initialHoursOfDay,
			int initialMinutes, int initialSeconds, boolean is24HourMode,
			boolean vibrate) {
		if (mTimeInitialized) {
		...
		mHideAmPm = DateTimeUtils
				.isTouchExplorationEnabled(mAccessibilityManager) ? true
				: mIs24HourMode;
		...
		mSecondRadialTextsView.initialize(res, secondsTexts, null, mHideAmPm,
				false);
	...
	private void setItem(int index, int value) {
		if (index == HOUR_INDEX) {
			...
			mSecondRadialSelectorView.setSelection(secondDegrees, false, false);
	...
	private int reselectSelector(int degrees, boolean isInnerCircle,
		...
		boolean allowFineGrained = (!forceToVisibleValue && (currentShowing == MINUTE_INDEX))
				|| (!forceToVisibleValue && (currentShowing == SECOND_INDEX));
		if (allowFineGrained) {
	...
	private int getDegreesFromCoords(float pointX, float pointY,
		...
		if (currentItem == HOUR_INDEX) {
			...
			return mSecondRadialSelectorView.getDegreesFromCoords(pointX,
					pointY, forceLegal, isInnerCircle);
		} else {
			return -1;
		}
		...
		if (animate && (index != lastIndex)) {
			...
			if (Build.VERSION.SDK_INT >= 11) {
				...
				int minuteVisibility = (index == MINUTE_INDEX) ? View.VISIBLE
						: View.INVISIBLE;
				int secondVisibility = (index == SECOND_INDEX) ? View.VISIBLE
						: View.INVISIBLE;
				...
				mSecondRadialTextsView.setVisibility(secondVisibility);
				mSecondRadialSelectorView.setVisibility(secondVisibility);
			}
		}
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (mIsTouchingAmOrPm == AM || mIsTouchingAmOrPm == PM) {
				...
				boolean forceLegal = DateTimeUtils
